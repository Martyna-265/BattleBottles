...
class BattleGrid extends PositionComponent with HasGameReference<BattleShipsGame> {
  ...
  BattleGrid(this.opponent, this.fleetConfig)
  ...
  void regenerateGrid() {
    ...
    fleetConfig.forEach((sizeKey, count) {
      ...
      List<ShipType> availableTypes = ShipType.all.where((t) => t.size == size).toList();
  ...
  bool _tryPlaceRotatedShip(Ship ship, ShipType newType, int newHeadX, int newHeadY) {
    ...
    for (var p in newPoints) {
      if (p.x < 0 || p.x >= game.squaresInGrid || p.y < 0 || p.y >= game.squaresInGrid) return false;
